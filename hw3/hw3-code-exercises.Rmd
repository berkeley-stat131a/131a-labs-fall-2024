...
Putting it all together, we want to identify the **slope** and **intercept** that minimize the following expression:

$$ \underset{\text{slope}, \text{intercept}}{\mathrm{arg\,min}} \sum_{i=1}^n \left(y_i - (\text{intercept} + \text{slope}*x_i \right)^2$$

By taking [partial derivatives](https://en.wikipedia.org/wiki/Partial_derivative) with respect to the $\text{slope}$ and $\text{intercept}$ parameters, we can show the following formulas provide the optimal slope and intercept:
...
-   Regardless of your math background, it's critical to understand **why** we have the formulas above: These equations minimize the sum of squared vertical distances from each point to the line.
...
Then, using your function, print the optimal slope for each the two plots from Exercise 4.
